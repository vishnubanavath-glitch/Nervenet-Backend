...
from claude import get_claude_response, api_key
...
def render_svg_block(svg_str: str):
    ...
    st.iframe(src=data_url, height=initial_height)
        ...
        if part_stripped.lower().startswith('<chart_data>') and part_stripped.lower().endswith('</chart_data>'):
            ...
            with st.spinner("Generating SVG Visualization..."):
                try:
                    ...
                    # Normalize HTML-escaped angle brackets on the SVG code for decryption matching
...
if user_input := st.chat_input("Write a message..."):
    ...
    if not api_key:
        st.error("Cannot send message: Anthropic API Key is missing. Please set `CLAUDE_API` in your `.env` file and refresh.")
    else:
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner(""):
                try:
                    # Pass the entire history to Claude and get the response text, debug info, and complete updated history
                    response, debug_info, updated_history = get_claude_response(messages, st.session_state.privacy_engine, model="claude-opus-4-8")
